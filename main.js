...
var struct_general = {  // Generic Container
    ...
    "per_time": [20, 20, 5, 5],
    "play_time": [0, 0, 0, 0],
    "metric_name": ["Remates", "Passes negativos", "Perdas de bola"],
...
function updateLiveButtons() {
    ...
    if (trackingData.length) {
        struct_team.players.forEach(function(p, idx){
            if (trackingData[idx]) {
                trackingData[idx].name = (p.nlast && p.nlast !== 'Player') ? p.nlast : 'Jogadora ' + (idx+1);
            }
        });
        renderTrackingBoard();
    }
...
function initializeTrackingData() {
  trackingData = struct_team.players.map(function(p, idx){
    var name = (p.nlast && p.nlast !== 'Player') ? p.nlast : 'Jogadora ' + (idx+1);
    return { name:name, r1:0, re1:0, r2:0, re2:0, pz1:0, pz2:0, pz3:0, pz4:0, pb1:0, pb2:0, pb3:0, pb4:0 };
  });
  renderTrackingBoard();
}
function renderTrackingBoard() {
  var el = document.getElementById('trackingRows');
  if (!el) return;
  el.innerHTML = '';
  trackingData.forEach(function(player, idx){
    var row = document.createElement('div');
    row.className = 'tracking-row';
    var html = '<div class="tracking-name-cell">' + player.name + '</div>';
    trackingCols.forEach(function(col){
      html += '<button class="tracking-stat ' + col.cls + '" onclick="changeTrackingValue(' + idx + ','' + col.key + '',1)">' + player[col.key] + '</button>';
    });
    row.innerHTML = html;
    el.appendChild(row);
  });
}
function changeTrackingValue(playerIdx, key, delta) {
  if (!trackingData[playerIdx]) return;
  trackingData[playerIdx][key] = Math.max(0, Number(trackingData[playerIdx][key] || 0) + delta);
  renderTrackingBoard();
}
//#endregion


if (document.getElementById("trackingRows")) { initializeTrackingData(); }
if (typeof displayClock === "function") { displayClock(clockMain, minutesM, secondsM, 0, 0); }
